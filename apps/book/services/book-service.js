  ...
  return new Promise((resolve, reject) => {
      try {
          ...
          if (!data) {
              ...
              resolve(
                  data.filter(
                      datum => (datum && datum.title.includes(searchTerm)) ||
                          (datum && datum.subtitle.includes(searchTerm)) ||
                          (datum && datum.authors.includes(searchTerm)) ||
                          (datum && datum.description.includes(searchTerm))
                          (datum && datum.categories.includes(searchTerm))
                  )
              )
          }
      } catch (error) {
...
function _add(bookToAdd) {
  let books = _loadFromStorage()
  const book = _createBook(bookToAdd.title, bookToAdd.price)
  books = [book, ...books]
  _saveToStorage(books)
  return Promise.resolve()
}

function _update(bookToUpdate) {
  let books = _loadFromStorage()
...
function _createBook(title, price = utilService.getRandomIntInclusive(1, 200)) {
  return {
...
function addGoogleBook(googleBook) {
  _createGoogleBookPreview(books)
  let books = _loadFromStorage()
  const book = _createBook(bookToAdd.title, bookToAdd.price)
  books = [book, ...books]
  _saveToStorage(books)
...
function _createGoogleBookPreview(title, img) {
  return {
    title: items[0].volumeInfo.title,
    img: items[0].volumeInfo.imageLinks.thumbnail
  }
}

function getBookFromAxios(searchValue) {
  return new Promise(function(resolve, reject) {
    const API_KEY = 'AIzaSyCI5V_bt3RgLAhFJcbBjfCLLPbpIrtNNhc'
    const URL = `https://www.googleapis.com/books/v1/volumes?q=${searchValue}&key=${API_KEY}`
    axios.get(URL)
        .then(res => {
            const googleBooks = res.data
            resolve(googleBooks)
